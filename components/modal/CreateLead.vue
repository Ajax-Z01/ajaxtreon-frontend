...
import type { LeadCreateInput, LeadStatus } from '~/types/Lead'

const leadStatuses: LeadStatus[] = ['new', 'contacted', 'qualified', 'lost', 'converted']

const props = defineProps<{ showModal: boolean }>()
const emit = defineEmits<{
  (e: 'createLead', payload: LeadCreateInput): void
  (e: 'closeModal'): void
}>()

const form = ref<LeadCreateInput>({
  name: '',
  email: '',
  phone: '',
  company: '',
...
<template>
  ...
  >
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-xl w-full max-w-2xl relative border border-gray-200">
      ...
      <!-- Form -->
      <form @submit.prevent="createNewLead" class="space-y-4">
        <!-- Name -->
        <div>
